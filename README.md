...
The active interface and space presentation use the Observatory visual direction. Multiplayer now has a functional six-player ENet lobby (host/join, readiness and disconnect handling); synchronized campaigns remain a separate milestone. See [OBSERVATORY.md](OBSERVATORY.md) for controls, visual architecture, tests and the multiplayer roadmap.

## Active Scene Flow

1. `project.godot` starts at `res://scene/MainMenue/MainUI.tscn`.
2. `scene/MainMenue/MainUI.gd` hosts the landing page, empire presets, settings, music controls, and direct-connect multiplayer lobby.
...
- **Station build costs are committed**: both the body-targeted gameplay flow and the shipyard flow charge construction costs. Low-level debug callers can still explicitly opt out.
...
- **Multiplayer lobby is implemented**: six-player direct-connect ENet, server-owned slots, readiness, protocol checks and disconnect handling. Campaign replication and a gameplay command/permission layer are not implemented yet.
