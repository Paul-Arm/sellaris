...
if (!Number.isInteger(seconds) || seconds < 10 || seconds > 3600)
  throw new Error('Duration must be 10..3600 seconds');
...
for (const args of runs) {
  ...
  await new Promise((resolve, reject) => {
    const child = spawn(
      process.execPath,
      ['--import=tsx', 'backend/benchmark.ts', ...args, `--seconds=${seconds}`],
      {
        cwd,
        windowsHide: true,
        stdio: 'inherit',
    ...
    child.once('exit', (code, signal) =>
      code === 0 ? resolve() : reject(new Error(`Benchmark stopped: ${signal || code}`)),
    );
  });
}
