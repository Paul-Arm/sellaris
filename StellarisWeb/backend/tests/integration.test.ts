  ...
  async (t) => {
    ...
    try {
      ...
      await t.test('one initial snapshot includes coherent fleets, ships, clock and battle', () => {
        ...
        assert.equal(Number(a.conn.db.battleVitals.count()), 40);
        assert.equal(checkCompactBattle(a, 1), 40);
        ...
        const ids = new Set([...a.conn.db.battleVitals.iter()].map((p) => p.shipId));
        assert([...a.conn.db.battleVitals.iter()].every((p) => !p.targetId || ids.has(p.targetId)));
      ...
      await t.test(
        ...
        async () => {
          ...
          for (const table of ['visible_battles', 'battle_participants']) {
            const retired = await sql(database, adminToken(), `SELECT * FROM ${table}`);
            assert(!retired.ok, `Retired view ${table} must not exist`);
          }
          ...
          assert.equal(Number(outsider.conn.db.battleVitals.count()), 0);
          assert.equal(checkCompactBattle(outsider, 1), 0);
          for (const table of [
            'battle_roster',
            'battle_motion',
            'battle_vitals',
      ...
      await t.test(
        ...
        async () => {
          ...
          try {
            ...
            assert.equal(checkCompactBattle(a, 1), 0);
      ...
      await t.test(
        ...
        async () => {
          ...
          await waitFor(() => Number(a.conn.db.battleVitals.count()) === 30, 'Withdrawal snapshot');
          assert.equal(checkCompactBattle(a, 1), 30);
      ...
      await t.test(
        ...
        async () => {
          ...
          const before = [...a.conn.db.focusedBattle.iter()][0].step;
          await delay(300);
          assert.equal([...a.conn.db.focusedBattle.iter()][0].step, before);
        },
      );
      await t.test(
      ...
      await t.test(
        ...
        async () => {
          ...
          assert([...resumed.conn.db.focusedBattle.iter()][0].step > before);
          assert.equal(
            [...resumed.conn.db.myEmpire.iter()][0].energy,
            10000,
            'no production before the first month',
          );
          assert.equal(Number(resumed.conn.db.fleetShips.count()), 10);
          assert.equal(Number(resumed.conn.db.battleVitals.count()), 30);
          assert.equal(checkCompactBattle(resumed, 1), 30);
          assert([...resumed.conn.db.battleVitals.iter()].some((p) => p.targetId > 0));
      ...
      await t.test('battle end atomically removes all compact detail rows', async () => {
        ...
        await waitFor(() => b.conn.db.focusedBattle.id.find(1)?.state === 'finished', 'Battle must finish');
        assert.equal(checkCompactBattle(b, 1), 0);
        await waitFor(
          () => clients.at(-1)!.conn.db.battleRoster.count() === 0n,
          'Other client receives battle end',
        );
        assert.equal(checkCompactBattle(clients.at(-1)!, 1), 0);
        await admin.conn.reducers.setClock({ paused: false, speed: 4 });
      });
