...
export function EmpireSidebar({
  ...
  onColony: (colony: ColonyEntry) => void;
  onFleet: (fleet: Fleet) => void;
  onOpenColony: (colony: ColonyEntry) => void;
  onOpenFleet: (fleet: Fleet) => void;
  section: 'colonies' | 'ships';
  onSection: (section: 'colonies' | 'ships') => void;
  onEvents: () => void;
  onLog: () => void;
}) {
  const pending =
    game.situations?.filter((item) => item.state.status === 'decision' || item.state.notice > item.state.seen)
      .length || 0;
  const crisis = game.crises?.find((c) => c.phase !== 'contained' && c.phase !== 'dormant');
  return (
    <aside className={`left-panel inventory-sidebar ${open ? 'mobile-open' : ''}`}>
      <EmpireInventory
        key={`${game.code}:${game.me.id}`}
        game={game}
      ...
      <footer className="inventory-sidebar-footer">
        <div>
          <button onClick={onEvents}>
            <Radio size={13} /> Lage <b>{pending || (crisis ? '!' : 0)}</b>
          </button>
          <button onClick={onLog}>Protokoll</button>
        </div>
      </footer>
    </aside>
  );
