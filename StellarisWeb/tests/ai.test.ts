import { ORIGINS } from '../shared/empireCatalog';
...
test('only hosts can add AI, and AI uses a real reserved player slot', () => {
  ...
  assert.ok(
    bots.every(
      (p) => !p.online && p.resources.energy === 420 + ORIGINS[p.empire.design.origin].resources.energy,
    ),
  );
...
test('AI has no resource stipend and cannot progress a paused simulation', () => {
  ...
  bot.ai!.nextDecision = 1000;
  const rate = income(game, bot);
  advance(game, 30);
  for (const r of ['energy', 'minerals'] as const) assert.ok(Math.abs(bot.resources[r] - rate[r]) < 1e-8);
  assert.ok(
    Math.abs(bot.resources.data - rate.data) < 1e-8,
    'monthly income includes idle Compute synthesis exactly once',
  );
...
test('AI autonomously explores, colonizes, builds and researches through ordinary commands', () => {
  ...
  assert.ok(bot.techs.length >= 1);
  assert.ok(
    bot.techs.length + bot.research.projects.length >= 2,
    'AI continues its monthly research program',
  );
