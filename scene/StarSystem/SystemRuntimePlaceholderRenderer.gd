...
func render_runtime_placeholders(space_renderables: Dictionary, outer_radius: float) -> Dictionary:
	...
	for ship_variant in ships_variant:
		...
		elif not str(ship_record.get("fleet_id", "")).is_empty() and fleet_ship_ids.has(str(ship_record.get("ship_id", ""))):
			continue
		else:
	...
	if not fleets.is_empty():
		...
		resolved_outer_radius = maxf(resolved_outer_radius, fleet_radius + 9.0)

	if not mobile_ships.is_empty():
		var ship_radius: float = outer_radius + SHIP_RING_OFFSET
		result["ships"] = _build_group(
			mobile_ships,
			"ship_id",
			ship_radius,
			Vector3(1.0, 1.0, 1.6),
