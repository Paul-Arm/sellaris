...
func render_hyperlanes() -> void:
	...
	for link in _host.hyperlane_links:
		...
		surface_tool.add_vertex(_host.system_positions[link.x])
		surface_tool.set_color(Color(0.32, 0.56, 0.95, 0.42))
		surface_tool.add_vertex(_host.system_positions[link.y])

	_host.hyperlanes.mesh = surface_tool.commit()
	_host.hyperlanes.material_override = lane_material
