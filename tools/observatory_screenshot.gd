...
func _ready() -> void:
	...
	var menu := load("res://scene/MainMenue/MainUI.tscn").instantiate()
	add_child(menu)
	await _capture("observatory_menu")
	menu._show_page(menu.PAGE_MULTIPLAYER)
	await _capture("observatory_lobby")
	menu.queue_free()
	await get_tree().process_frame
	var setup := load("res://scene/GennerateMenue/GennerateMenue.tscn").instantiate()
	add_child(setup)
	await _capture("observatory_setup")
	setup.queue_free()
	await get_tree().process_frame
	var game := load("res://scene/game/GameScene.tscn").instantiate()
